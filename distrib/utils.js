    ...
    class Utils {
        ...
        */
        static scrollMemoryTable(pcb) {
            ...
            if (rowToScroll <= TOP_THREE || rowToScroll >= BOTTOM_THREE)
        ...
        */
        static printTime(pcb) {
            ...
            _StdOut.advanceLine();
            if (pcb.turnaroundTime != 1)
                _StdOut.putText(`Turn Around Time: ${pcb.turnaroundTime} cycles.`);
            else
                _StdOut.putText(`Turn Around Time: ${pcb.turnaroundTime} cycle.`);
            _StdOut.advanceLine();
            if (pcb.waitTime != 1)
                _StdOut.putText(`Wait Time: ${pcb.waitTime} cycles.`);
            else
                _StdOut.putText(`Wait Time: ${pcb.waitTime} cycle.`);
            _StdOut.advanceLine();
